...
def contract():
    ...
    for i in noCtract:
        if i[4] == "매수" and i[6]/10000 >= nowPrice[coins[i[2]-1][1]]:
            ...
            ap = (walletData[4]*walletData[3] + nowPrice[coins[i[2]-1][1]]*i[5]*10000)/((walletData[3]+i[5])) #(wallet 평균가 * wallet 코인 개수 + 새로 살 떄 든 돈 ) / (지갑에 코인 양 + 주문한 코인 양)
            print(ap)
            #코인 평균가 계산
            ...
            cur.execute("UPDATE wallet_list SET wallet_coin_amnt = wallet_coin_amnt + {val0} , wallet_aver_price = {val1}\
            ...
            cur.execute("UPDATE wallet_list SET wallet_coin_amnt = wallet_coin_amnt - {val0} \
                WHERE coin_pk = 0 and user_pk = {val3}".format(val0=nowPrice[coins[i[2]-1][1]]*i[5]/10000, val3=i[1]))
            #wallet에서 원화 감소
            conn.commit()
            ...
            cur.execute("UPDATE wallet_list SET wallet_coin_amnt = wallet_coin_amnt + {val0} WHERE coin_pk = 0 and user_pk ={val2}".format(val0=(i[5]*nowPrice[coins[i[2]-1][1]]/10000), val2=i[1]))
            #wallet 에서 원화 증가
